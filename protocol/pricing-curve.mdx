...
<p style={{ marginBottom: '1.5rem' }}>
This mechanism applies symmetrically to both unlocking and redemption using identical mathematical functions. All participants interact with the same pricing logic, enforced by immutable onchain code with no administrative override capability. Prices cannot be altered, manipulated, or manually adjusted by any party. While MNY may also trade on external secondary markets, the Reserve's deterministic pricing serves as an anchor that bounds market variance through natural arbitrage dynamics. All transaction outcomes are independently verifiable and reproducible using the formulas and constants documented below.
</p>
...
<p style={{ marginBottom: '1.5rem' }}>
While the Moony Reserve serves as the primary venue for MNY price discovery, tokens may also trade on external secondary markets such as decentralized exchanges (DEXs) or centralized trading platforms. On these venues, price discovery follows traditional market mechanics - order book matching, AMM pool ratios, or other liquidity-based mechanisms - which can produce prices that temporarily diverge from the Reserve's deterministic curve.
</p>

<p style={{ marginBottom: '1.5rem' }}>
However, the Reserve's guaranteed liquidity and deterministic pricing create a powerful anchoring effect through arbitrage. When secondary market prices deviate from the Reserve price, rational arbitrageurs can extract risk-free profit by trading against the discrepancy:
</p>
...
<p style={{ marginBottom: '1.5rem' }}>
This arbitrage mechanism effectively establishes the Reserve as both a price floor and price ceiling for external markets. The magnitude of sustainable deviation is bounded by transaction costs (gas fees, exchange fees, slippage) - any price gap exceeding these costs becomes an arbitrage opportunity that market participants will exploit until equilibrium is restored.
</p>

<p style={{ marginBottom: '1.5rem' }}>
The economic implication is significant: regardless of external market volatility, sentiment-driven speculation, or temporary liquidity imbalances on secondary venues, MNY prices across all markets remain tethered to the Reserve's deterministic curve. This provides participants with price stability guarantees that traditional tokens - whose value depends entirely on secondary market dynamics - cannot offer.
</p>
